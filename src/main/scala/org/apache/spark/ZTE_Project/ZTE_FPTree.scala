 ...
 * @tparam T item type
 */
private[ZTE_Project] class ZTE_FPTree[T] extends Serializable {
  ...
  def merge(other: ZTE_FPTree[T]): this.type = {
  ...
  private def project(suffix: T): ZTE_FPTree[T] = {
    val tree = new ZTE_FPTree[T]
...
private[ZTE_Project] object ZTE_FPTree {
