...
object ZTE_fpg_demo {
  ...
  def run(params: Params) {
    ...
    val result = model.freqItemsets.collect().map(itemset => (itemset.items.mkString("[", ",", "]"), itemset.freq))

    sc.makeRDD(result).saveAsTextFile("/home/yang/result")
